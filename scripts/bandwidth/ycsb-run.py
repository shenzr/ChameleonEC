...
def ycsbOnly():
    ...
    for r in ycsb_requestors:
        #command = "ssh " + "root@"+ node + " \"" + ycsbHome + "/bin/ycsb run hbase20 -P " + ycsbHome + "/workloads/workloada -cp " + hbaseHome + "/conf/ -p columnfamily=family -p operationcount=1000000 -threads 4\" "
        # command = "ssh " + r + " \" " + ycsbHome + "/bin/ycsb run  hbase20 -P " + ycsbHome + "/workloads/workloada -cp " + hbaseHome +"/conf/ -threads 4\" "
        command = "ssh " + r + " \" " + ycsbHome + "/bin/ycsb run  hbase20 -P " + ycsbHome + "/workloads/testworkload -cp " + hbaseHome +"/conf/ -threads 4\" "
        print(command)
...
def ycsbAndRepair():
    ...
    for r in ycsb_requestors:
        #command = "ssh " + "root@"+ node + " \"" + ycsbHome + "/bin/ycsb run hbase20 -P " + ycsbHome + "/workloads/workloada -cp " + hbaseHome + "/conf/ -p columnfamily=family -p operationcount=1000000 -threads 4\" "
        command = "ssh " + r + " \" " + ycsbHome + "/bin/ycsb run  hbase20 -P " + ycsbHome + "/workloads/testworkload -cp " + hbaseHome +"/conf/ -threads 4\" "
        #command = "ssh " + r + " \" " + ycsbHome + "/bin/ycsb load  hbase20 -P " + ycsbHome + "/workloads/testworkload -cp " + hbaseHome + "/conf/ -threads 2\" "
        print(command)
...
if __name__ == '__main__':    
    ...
    for node in ycsb_requestors:
        command = "scp " + ycsbHome + "/workloads/testworkload " + node + ":" + ycsbHome + "/workloads/"
        print(command)
        os.system(command)
