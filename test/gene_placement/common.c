...
void gene_rndm_plcement(int ecK, int ecM, int* placement, int stripe_num, int node_num, int rack_num) {
    ...
    for(i=0; i<stripe_num; i++) {
	    ...
	    for(chnk_id=0; chnk_id<ecK+ecM; chnk_id++){
           ...
           if(chunk_num_rack[rack_id]>=ecM){
               chnk_id--; 
               continue;
           }

	       // update the node_id and remain_node_num
	       placement[i*(ecK+ecM)+chnk_id]=node_id[rndm_nd_id];
	...
	// print the placement
	//print_matrix(ecK+ecM, stripe_num, placement);
    free(chunk_num_rack);
    free(node_id);
}
