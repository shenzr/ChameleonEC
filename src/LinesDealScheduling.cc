...
void initInAndOutPointer(int** p, int n, vector<int>& deg) {
    cout << "deg: " << endl;
    for(int i=0; i<deg.size(); ++i) cout << deg[i] << " ";
    cout << endl;
    for(int i=0; i<n; ++i) {
        p[i] = (int*)malloc(sizeof(int) * (deg[i] + 1));
        p[i][0] = deg[i];
    }
}
...
void LinesDealScheduling::buildGraph(int repair_method, repairGraph& G, int ecK) {
    if(repair_method == SINGLE_STRIPE_PPR) { // -- ppr
        ...
        vector<int> indeg(ecK+1, 0), outdeg(ecK+1, 1);
        indeg[ecK] = ecK, outdeg[ecK] = 0;
        initInAndOutPointer(G.in, ecK+1, indeg);
        initInAndOutPointer(G.out, ecK+1, outdeg);
        for(int i=0; i<ecK; ++i) {
            G.in[ecK][i+1] = i;
            G.out[i][1] = ecK;
        }
    } else if(repair_method == SINGLE_STRIPE_PATH) { // -- linear path (ecpipe/PUSH)
        vector<int> indeg(ecK+1, 1), outdeg(ecK+1, 1);
        indeg[0] = 0, outdeg[ecK] = 0;
        initInAndOutPointer(G.in, ecK+1, indeg);
        initInAndOutPointer(G.out, ecK+1, outdeg);
        for(int i=0; i<ecK; ++i) {
            G.in[i+1][1] = i;
...
/*
    cout << "start buildStripeGraph" << endl;
    int K = _ecK;
    ...
    G._node_cnt = K+1;
    G.in = (int**)malloc(sizeof(int*) * G._node_cnt);
    G.out = (int**)malloc(sizeof(int*) * G._node_cnt);
...
*/
    ...
    int nr = _ec_LRC_L ? _ecK/_ec_LRC_L : 0 ;
    repairGraph lg(nr+1);
    lg.in = (int**)malloc(sizeof(int*) * lg._node_cnt);
    lg.out = (int**)malloc(sizeof(int*) * lg._node_cnt);
    buildGraph(repair_method, lg, nr);
    localG = lg;
}
...
cout << "ppr_graph" << endl;
...
#endif
    vector<int> indeg(ecK+1, 0), outdeg(ecK+1, 0);
    for(int j=1; j<=depth; ++j) {
        int st = 1<<(j-1);
        int stride = st;
        for(int k=st; k<=ecK; k+=2*stride) {
    ...
    for(int i=0; i<ecK+1; ++i) G.in[i] = (int*)malloc(sizeof(int) * (indeg[i]+1)), G.in[i][0] = 0;
    for(int i=0; i<ecK+1; ++i) G.out[i] = (int*)malloc(sizeof(int) * (outdeg[i]+1)), G.out[i][0] = 0;
...
vector<Stripe> LinesDealScheduling::repairboost(map<int, int>& isNodeIdFail, int repair_method, int lostBlkCnt, int* idxs, int* placement, int* isSoureCandidate) {
    ...
    for(int i=0; i<sourceVertexs.size(); ++i) {
        ...
        for(int j=0; j<_ecN; ++j) {
            ...
            if(isNodeIdFail[pid] || isSelected.count({sid, pid})) continue;
    ...
    for(int i=0; i<lostBlkCnt; ++i) {
        ...
        for(int j=0; j<_slaveCnt; ++j) {
            // if(isStore.count(j)) continue; (upd 22.5.3 for multi-fail)
            if(isStore.count(j) || isNodeIdFail[j]) continue;
    ...
    unitTestForMapping(stripes, placement, isSoureCandidate, isNodeIdFail, lostBlkCnt);
    ...
    // unitTestForSchedulingScheme(stripes, SCHEDULE_METHOD_BOOST, failNodeId, lostBlkCnt, upload_to, download_from);
    unitTestForSchedulingScheme(stripes, SCHEDULE_METHOD_BOOST, lostBlkCnt, upload_to, download_from);
...
void LinesDealScheduling::alterRepairGraph(int round, vector<Stripe>& stripes, map<pair<int, int>, int>& edgeMap, vector<pii>& up, vector<pii>& dw) {
    ...
    for(int i=isap.edge_st_ed[1][0]; i<isap.edge_st_ed[1][1]; i+=2) {
        if(isap.edges[i].flow) {
            ...
            stripes[sid].repairOrder[from_in_g][to_in_g] = round;
...
void LinesDealScheduling::unitTestForMapping(vector<Stripe>& stripes, int* placement, int* isSoureCandidate, map<int, int>& isNodeIdFail, int lostBlkCnt) {
    int p;
...
void LinesDealScheduling::unitTestForSchedulingScheme(vector<Stripe>& stripes, int schedule_method, int lostBlkCnt, vector<vector<pii> >& upload_to, vector<vector<pii> >& download_from) {
...
vector<Stripe> LinesDealScheduling::random_select(map<int, int>& isNodeIdFail, int repair_method, int lostBlkCnt, int* idxs, int* placement, int* isSoureCandidate) {
    ...
    for(int i=0; i<lostBlkCnt; ++i) {
        ...
        for(int j=0; j<_ecN; ++j) {
            ...
            if(isNodeIdFail[placement[i*_ecN+j]]) continue;
        ...
        p = 0;
        for(int j=0; j<_slaveCnt; ++j) {
            // if(vis[j]) continue; (upd 22.5.3 for multi-fail)
...
vector<Stripe> LinesDealScheduling::least_recent_selected(map<int, int>& isNodeIdFail, int repair_method, int lostBlkCnt, int* idxs, int* placement, int* isSoureCandidate) {
    cout << typeid(this).name() << "::" << __func__ << " starts\n";
    ...
    for (int i = 0; i < lostBlkCnt; ++i) {
        ...
        for(int j = 0; j < _slaveCnt && cnt < localK; ++j) {
            ...
            if(isNodeIdFail[pid] || !vis[pid]) continue;
        ...
        for(int j = 0; j < _slaveCnt; ++j) {
            pid = timestamp[j].second;
            // if(!vis[pid] && !isNodeIdFail[pid]) { (upd 22.5.3 for multi-fail)
            if(!vis[pid] && !isNodeIdFail[pid]) {
                timestamp[j].first = i + 1;
                change[pid].push_back(i+1);
...
void LinesDealScheduling::heterogeneous_alter_repairGraph(int fail_node_id, int* bandwidth_weight, int round, vector<Stripe>& stripes, map<_piii, int>& edgeMap, vector<pii>& up, vector<pii>& dw, int** cur_slave_load) {
    ...
    for(int i=0; i<peers.size(); ++i) {
        ...
        if(ok) {
            ...
            stripes[sid].repairOrder[vfrom_to[0]][vfrom_to[1]] = round;
