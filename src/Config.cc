...
Config::Config(std::string confFile) {
    ...
    for (element =
        ...
        else if (attName == "node.fail.cnt")
            _failNodeCnt = std::stoi(ele->NextSiblingElement("value")->GetText());
        else if (attName == "period.len")
            _periodLen = std::stoi(ele->NextSiblingElement("value")->GetText());
        ...
        else if (attName == "single.stripe.alg") {
        ...
        else if(attName == "schedule.method") {
            std::string s = ele->NextSiblingElement("value")->GetText();
            if(s == "online") _scheduleMethod = SCHEDULE_METHOD_ONLINE;
            else if(s == "boost") _scheduleMethod = SCHEDULE_METHOD_BOOST;
            else if(s == "random") _scheduleMethod = SCHEDULE_METHOD_RANDOM;
        }
